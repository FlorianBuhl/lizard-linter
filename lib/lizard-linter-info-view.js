...
export default class LizardInfoView {
  ...
  constructor(content) {
    ...
    sheet.insertRule('table, th, td { border: 1px solid black; border-collapse: collapse; }', 1);

    sheet.insertRule('th, td {text-align: center; padding: 5px;}', 2);
    ...
    for (let i = 0; i < content.functionAnalysis.length; i += 1) {
      ...
      td.textContent = `${content.functionAnalysis[i].name} @line ${content.functionAnalysis[i].lineStart}-${content.functionAnalysis[i].lineEnd}`;
      tr.appendChild(td);
