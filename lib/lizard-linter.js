...
/* Executes the lizard tool by command line interface for a given filepath */
...
// Analyses the reported stdout of the lizard tool.
...
export function activate() {
  subscriptions = new CompositeDisposable(
    ...
    atom.workspace.addOpener((uri) => {
      if (uri === 'atom://lizard-linter') {
        if (lizardInfoView === undefined) {
          console.info('create new lizard info view');
          lizardInfoView = new LizardInfoView(fileAnalysis);
        } else {
          console.info('update lizard info view');
    ...
    atom.commands.add('atom-workspace', {
      ...
      'lizard-linter:executeLint': () => this.executeLint(),
    }),
...
/* executes manually the lint for the current opened file */
/* try for jira connection another try another try with tracking */
export async function executeLint() {
  const activeTextEditor = atom.workspace.getActiveTextEditor();

  if (activeTextEditor) {
    console.info(`execute on ${activeTextEditor.getPath()}`);
    await lint(activeTextEditor);
    atom.workspace.toggle('atom://lizard-linter');
  }
}
